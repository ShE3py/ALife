...
if(supported) {
    ...
    const imports = {
        common: {
            ...
            createShader: (filenamePtr, shaderType, configFilenamePtr) => {
                ...
                const configFilename = readCstr(configFilenamePtr);
                let config;
                
                const filename = readCstr(filenamePtr);
                let src;
        ...
        env: {
            ...
            glGetProgramInfoLog: (program, maxLength, lenPtr, logPtr) => {
                ...
                var utf8 = unescape(encodeURIComponent(log));
    ...
    WebAssembly.instantiateStreaming(fetch("physarum.wasm"), imports).then(
        (result) => {
